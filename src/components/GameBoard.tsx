          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-3 lg:p-5 rounded-[20px] shadow-2xl flex-1 flex flex-col relative overflow-hidden">
             ...
             <div className="flex flex-row lg:flex-col overflow-x-auto lg:overflow-y-auto lg:space-y-2.5 gap-2 lg:gap-0 relative z-10 pr-1 pb-2 lg:pb-0 custom-scrollbar items-center lg:items-stretch">
                {room.players.map((p: any) => {
                  const rankedPlayers = [...room.players].sort((a: any, b: any) => b.score - a.score);
                  const idx = rankedPlayers.findIndex((player: any) => player.id === p.id);
          ...
          <div className="flex flex-wrap md:flex-nowrap items-center justify-between gap-y-2 px-2">
              ...
              <div className="flex items-center gap-3 md:gap-4 bg-linear-to-r from-fuchsia-900/50 to-indigo-900/50 border border-fuchsia-500/30 px-4 py-1.5 md:py-2 rounded-xl shadow-[0_0_15px_rgba(232,121,249,0.15)] flex-1 md:mx-4 mx-1 justify-center order-last md:order-none w-full md:w-auto">
                 ...
                 <div className="text-left">
                    ...
                    <span className="block text-base font-black text-white drop-shadow-md">
                      #{rankedPlayers.findIndex((p: any) => p.id === localPlayerId) + 1}
                    </span>
                 </div>
              </div>
             ...
             >
               ...
               {Array.from({ length: yMax }).map((_, y) => (
                 Array.from({ length: xMax }).map((_, x) => {
                   ...
                   const guessers = Object.entries(gameState.guesses).filter(([_, g]: any) => g.x === x && g.y === y).map(([id]) => room.players.find((p: any) => p.id === id)?.name);
                   ...
                   return (
                     ...
                     >
                       ...
                       {/* Guess marker dots */}
                       {(guessers.length > 0 && isScorePhase) && (
                         <div className="absolute inset-0 flex flex-wrap items-center justify-center gap-px p-px z-10">
                           {guessers.map((g, i) => (
                             <div key={i} className="w-[5px] h-[5px] md:w-2 md:h-2 bg-white rounded-full shadow-[0_0_6px_rgba(0,0,0,1)] border border-black/50" title={g} />
                           ))}
                         </div>
                       )}
                     </div>
               ...
               {gameState.phase === 'SCORE_PHASE' && gameState.targetColor && (() => {
                 ...
                 return (
                   <>
                     <div 
                       className="rounded-md z-30 pointer-events-none flex items-start justify-end transition-all duration-500"
                       style={{ 
                     ...
                     >
                       <div className="bg-[#070b1a] text-cyan-400 font-bold px-1.5 py-0.5 rounded-bl-md text-[9px] md:text-[10px] border-b border-l border-cyan-500/50 backdrop-blur-md shadow-[-2px_2px_10px_rgba(34,211,238,0.3)]">+1 pt</div>
                     </div>
                     <div 
                       className="rounded-md z-40 pointer-events-none flex items-start justify-end transition-all duration-500"
                       style={{ 
                     ...
                     >
                       <div className="bg-white text-fuchsia-700 font-black px-1.5 md:px-2 md:py-0.5 rounded-bl-md text-[10px] md:text-xs tracking-wider shadow-[-2px_2px_15px_rgba(232,121,249,0.5)]">+2 pts</div>
                     </div>
                   </>
                 );
               })()}
       ...
       <div className="hidden lg:flex w-[240px] flex-col gap-4">
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-5 rounded-[20px] shadow-2xl flex-1 flex flex-col relative overflow-hidden">
             ...
             <div className="flex-1 overflow-y-auto space-y-3 text-[13px] font-medium text-zinc-300 mb-3 pr-1 custom-scrollbar relative z-10">
                {gameState.phase === 'GUESS_PHASE' && (
                  <>
                    <div><span className="text-cyan-400 font-bold">{currentGiver.name}:</span> I think it&apos;s a shade of {gameState.clue}!</div>
                    <div><span className="text-fuchsia-400 font-bold">System:</span> Clue given. Start guessing!</div>
                  </>
                )}
                {gameState.phase === 'CLUE_PHASE' && (
                  <div className="text-zinc-500 italic">Waiting for clue...</div>
                )}
                {gameState.phase === 'SCORE_PHASE' && (
                  <div><span className="text-emerald-400 font-bold">System:</span> Scoring complete!</div>
                )}
             </div>
             ...
             <div className="relative z-10">
                <div className="flex gap-2">
                  <input type="text" placeholder="Type a message..." disabled className="flex-1 bg-black/40 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-zinc-300 shadow-inner focus:outline-none placeholder:text-zinc-600" />
                  <button disabled className="px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-zinc-500 text-xs font-bold disabled:opacity-50">▶</button>
                </div>
             </div>
          </div>
